...
class BYTETracker(object):
    ...
    def step(self, results, public_det=None):
        ...
        scores = np.array([item['score'] for item in results if item['class'] == 2], np.float32)
        bboxes = np.vstack([item['bbox'] for item in results if item['class'] == 2])  # N x 4, x1y1x2y2
